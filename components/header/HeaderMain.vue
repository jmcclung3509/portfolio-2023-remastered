<template>
	<header class="main flex w-full h-28"
		...
		<!-- <img class="h-[70px]" src="@/assets/images/home/circle-avatar.svg" alt="avatar"/>-->
				...
				<nav class="right main flex justify-end items-center">
					<ul class="flex z-10 items-center">
						...
						<li class="relative p-2">
							<a href="/about" class="animate-underline orange"
								>About</a>
						</li>
			...
			<template v-else>
				<div class="mobile-nav-trigger  ml-auto cursor-pointer fixed top-10 right-10"
					@click="(data.mobileMenuOpen = !data.mobileMenuOpen), $emit('mobileMenuClick', data.mobileMenuOpen)">
					<font-awesome-icon
				class="icon text-1.5xl" :class="data.mobileMenuOpen ? 'text-light-peach hover:text-orange' : 'text-default-dark hover:text-dark-purple'"
